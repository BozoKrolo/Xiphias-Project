...
swimLaneTemplate.innerHTML = `
    ...
    <div class="todo-item">
        ...
        <button class="delete-todo-item-btn">🗑️</button>
    </div>
`;
...
export class TodoItemComponent extends HTMLElement {
  ...
  connectedCallback() {
    const todoValue = this.getAttribute('data-value') || `Task ${this.id}`;
    const todoChecked = this.getAttribute('data-checked') === 'true';

    this.contentInput.value = todoValue;
    this.contentDisplay.innerHTML = this.contentInput.value;
    this.todoCheck.checked = todoChecked;
